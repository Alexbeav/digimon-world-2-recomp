...
#
# Writes: dist/digimonworld2-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix digimonworld2 \
  --exe-name Digimon_World_2 \
  --omit-openbios \
  --display-name "Digimon World 2" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Digimon World 2 (USA) disc" \
  --runtime-dir mods \
